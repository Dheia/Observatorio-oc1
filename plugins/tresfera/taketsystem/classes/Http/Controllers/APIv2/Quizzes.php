...
namespace Tresfera\Taketsystem\Classes\Http\Controllers\APIv2;

use Tresfera\Taketsystem\Classes\Http\Controllers\API;
use Request;
use Tresfera\Taketsystem\Models\Quiz;
use Tresfera\Buildyouup\Models\Answer;
...
class Quizzes extends API
{
    ...
    public function get()
    {
	  ...
	  if(post("ev")) {
			$sql = Result::where("evaluacion_id",post("ev"))
							->where("quiz_id",$quizzes[0]->id);
			if(post("team"))
				$sql->where("email",post("team"));
	  ...
	  if(post("team"))
	  	$result->email = post("team");
		if(post("ev"))
			$result->evaluacion_id = post("ev");
