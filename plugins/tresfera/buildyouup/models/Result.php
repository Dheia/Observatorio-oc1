...
class Result extends \Tresfera\Taketsystem\Models\Result
{
     ...
     */
    public $table = 'tresfera_buildyouup_results';

    public $hasMany = [
      'answers'  => ['Tresfera\Buildyouup\Models\Answer'],
    ];
    ...
    public function afterSave()
    {
        ...
        if(isset($evaluacion->id)) {
            ...
            if( count($eval_tmp["pendientes"])==0)
            {
                $evaluacion->estado = 2;
                $evaluacion->save();
                \Queue::push('\Tresfera\Buildyouup\Classes\Jobs\GenerateRapport',$evaluacion->id,"rapports");
            }
            else
            {
                $evaluacion->estado = 1;
