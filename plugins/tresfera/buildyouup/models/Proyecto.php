...
class Proyecto extends Model
{
    ...
    public $hasMany = [
      'gestores' => ['Tresfera\Buildyouup\Models\Gestor'],
      'equipos' => ['Tresfera\Buildyouup\Models\Equipo'],
    ];
    /**
     * @var string The database table used by the model.
     */
    public $table = 'tresfera_buildyouup_proyecto';
    public $attachOne = [
    ...
    {
      \Queue::later($this->fecha_fin, '\Tresfera\Buildyouup\Classes\Jobs\FinalizaEquipos', ['proyecto_id' => $this->id]);
    }
    */
    ...
    public static function getEstadisticas($lista)
    {
      ...
      if(isset($filters[0]['scope-proyecto']) and count($filters[0]['scope-proyecto'])) {
        ...
        if(!isset($query))
        {
            ...
            $query_proyectos = Proyecto::whereIn("tresfera_buildyouup_proyecto.id", $ids);
        }
        else
        {
            $query->whereIn("proyecto_id",$ids);
            $query_proyectos->whereIn("tresfera_buildyouup_proyecto.id",$ids);
        }
      }
      ...
      if(isset($filters[0]['scope-estado']) and count($filters[0]['scope-estado'])) {
          ...
          if(!isset($query))
          {
              ...
              $query_proyectos = Proyecto::join('tresfera_buildyouup_evaluacion', 'tresfera_buildyouup_proyecto.id', '=', 'tresfera_buildyouup_evaluacion.proyecto_id')
                                          ->whereIn('tresfera_buildyouup_evaluacion.estado', $ids)
                                          ->groupBy('tresfera_buildyouup_proyecto.id')
                                          ->select('tresfera_buildyouup_proyecto.name');
          }
          else
          {
              $query->whereIn("estado",$ids);
              $query_proyectos->join('tresfera_buildyouup_evaluacion', 'tresfera_buildyouup_proyecto.id', '=', 'tresfera_buildyouup_evaluacion.proyecto_id')
                              ->whereIn('tresfera_buildyouup_evaluacion.estado', $ids)
                              ->groupBy('tresfera_buildyouup_proyecto.id')
                              ->select('tresfera_buildyouup_proyecto.name');
          }
      ...
      if(isset($filters[0]['scope-evaluado']) and count($filters[0]['scope-evaluado'])) {
          ...
          if(!isset($query))
          {
              ...
              $query_proyectos = Proyecto::join('tresfera_buildyouup_evaluacion', 'tresfera_buildyouup_proyecto.id', '=', 'tresfera_buildyouup_evaluacion.proyecto_id')
                                          ->whereIn('tresfera_buildyouup_evaluacion.user_id', $ids);
          }
          else
          {
