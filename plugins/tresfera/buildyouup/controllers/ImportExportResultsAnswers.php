...
class ImportExportResultsAnswers //extends ImportExportController
{
    ...
    public function exportar()
    {
        try {
            $options = [];
            $defaultOptions = [
                'fileName' => "datosBrutosBuildyouup360_".date("d-m-Y").".csv",
                'delimiter' => ';',
                'enclosure' => '"'
            ];
            ...
            */
            ...
            $ans = Answer::join('tresfera_taketsystem_results', 'tresfera_taketsystem_results.id', 'tresfera_taketsystem_answers.result_id')
                        ->join('tresfera_buildyouup_evaluacion', 'tresfera_taketsystem_results.evaluacion_id', 'tresfera_buildyouup_evaluacion.id')
                        ->where('tresfera_buildyouup_evaluacion.lang','es')
                        ->where('tresfera_taketsystem_answers.question_type','select')
                        ->where("tresfera_taketsystem_results.evaluacion_id","<>",0)
                        ->select('tresfera_taketsystem_answers.*', 'tresfera_buildyouup_evaluacion.lang as idioma' )
