...
use Tresfera\Buildyouup\Models\Cuestionario;
...
class Proyectos extends Controller
{
    ...
    public function generateRapport($model_id) {
      ...
      $model = \Tresfera\Buildyouup\Models\Proyecto::with("equipos")->find($model_id); 
      $equipos = Session::get('buildyouup::proyecto::'.$model_id, $model->equipos->lists("id","name"));
      if(isset($equipos[0])) {
        $equipos = $equipos[count($equipos)-1];
      }

      $data = \Tresfera\Buildyouup\Models\Rapport::getDataRapport($equipos);
      ...
      if(count($equipos) == 1) { //si solo hay un equipo seleccionado
        foreach($equipos as $equipo => $id) {
          ...
          $equipoObj = \Tresfera\Buildyouup\Models\Equipo::find($id);
          $players = [];
      ...
      return PDF::loadTemplate('buildyouup',$data)
    ...
    public function onFilterUpdate($model_id) {
      ...
      Session::push('buildyouup::proyecto::'.$model_id, $equipos->lists("id","name"));
      
      $data = \Tresfera\Buildyouup\Models\Rapport::getDataRapport($equipos->lists("id","name"));
...
*/
    ...
    public function onSend() {
      $form = request();
      $equipo = \Tresfera\Buildyouup\Models\Equipo::find($form['id']);
      if(!isset($equipo->id)) {
      ...
      foreach($players as $player) {
        if(!isset($player['email']) || $player['email'] == "") continue;
          \Mail::queue('buildyouup.require.answer.evaluador', $data, function($message) use ($player)
          {
    ...
    public function onSaveEval()
    {
      ...
      return Redirect::to( Backend::url('tresfera/buildyouup/proyectos/update/'.$proyecto_id) );
