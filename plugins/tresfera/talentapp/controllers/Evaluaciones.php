...
class Evaluaciones extends Controller
{
    ...
    public function sendEvaluadores() {
      $evaluacion = Evaluacion::find(get("id"));
      $evaluadores = $evaluacion->getEvaluadores();
      $stats = $evaluacion->stats;

      foreach($evaluadores as $tipo=>$evaluadors) {
        if(is_array($evaluadors))
        foreach($evaluadors as $email=>$contacto) {
          if($email == get("email")) {
            $data = [
              "name" => $contacto['name'],
              "username" => $contacto['email'],
              "url" => $stats[$tipo][$contacto['email']]['url'],
              "date" => $evaluacion->proyecto->fecha_fin,
              "name_evaluado" => $evaluacion->name,
            ];
            \Mail::queue('talentapp.require.answer.evaluador', $data, function($message) use ($contacto)
            {
                $message->to($contacto['email'],$contacto['name']);
                //$message->to("fgomezserna@gmail.com",$contacto['name']);
            });
  ...
  /*  static function getDataRapport($id_evaluacion) {
   ...
   ->first()->value;
    ...
    public function showRapport() {
      $evaluacion = \Tresfera\Talentapp\Models\Evaluacion::find(get("id"));
      ...
      if(in_array('autoevaluacion',$evaluacion->tipo)) {
        ...
        return PDF::loadTemplate('talentapp:autoevaluacion',$data)
    ...
    public function formExtendFields($form,$record)
    {
      if(isset($this->params[0])) {
        $evaluacion = \Tresfera\TalentApp\Models\Evaluacion::find($this->params[0]);
      }
      if($this->user->is_superuser) return;
      if($this->user->role_id == 3 || true) {
        
        if(isset($evaluacion->tipo))
          foreach($record['tipo']->config['options'] as $name=>$null) {
            if(is_array($evaluacion->tipo))
