...
class ImportExportResultsAnswers //extends ImportExportController
{
    ...
    public function exportar($id_proyecto)
    {
        try {
            $proyecto = Proyecto::find($id_proyecto);
            $options = [];
            $defaultOptions = [
                'fileName' => "datosBrutosTalentApp360_".str_slug($proyecto->name)."_".date("d-m-Y").".csv",
                'delimiter' => ';',
                'enclosure' => '"'
            ];
            ...
            */
            ...
            $evaluaciones_list = Evaluacion::where("proyecto_id",$id_proyecto)->lists("id","id");
            

            $results = Result::join('tresfera_taketsystem_answers', 'tresfera_taketsystem_results.id', 'tresfera_taketsystem_answers.result_id')
                            ->whereIn("tresfera_taketsystem_results.evaluacion_id",$evaluaciones_list)
                            ...
                            ->where("tresfera_taketsystem_results.uncompleted",0)
            ...
            //echo "Empezamos a hacer cabeceras"."\n";
            $headers = [];
            ...
            foreach($results as $result)
            {
                ...
                $ev = Evaluacion::find($result->evaluacion_id);
                ...
                foreach($competencia as $name_competencia => $slides)
                {
                    ...
                    foreach($slides as $slide_id)
                    {
                        ...
                        if($name_competencia == "Visión Estratégica")
                        {
                            if($i == 3) {
                                array_push($headers[0], $dimension);
                                array_push($headers[1], $name_competencia."3");
                                $i++;
                            }
                            if($i == 4) break;
                        }
            ...
            $csv->insertOne($h1);
            $csv->insertOne($h2);
            //echo "Acabamos a hacer cabeceras"."\n";
            //print_r($h2);
            ...
            /*$results = Result::join('tresfera_taketsystem_answers', 'tresfera_taketsystem_results.id', 'tresfera_taketsystem_answers.result_id')
                            ...
                            ->take(2)
                            ->get();*/
            $results = Result::where("tresfera_taketsystem_results.duplicated",0)
                            ->whereIn("tresfera_taketsystem_results.evaluacion_id",$evaluaciones_list)
                            ->where("tresfera_taketsystem_results.evaluacion_id","<>",0)
                            ->where("tresfera_taketsystem_results.uncompleted",0)
                            ->orderBy("tresfera_taketsystem_results.id","asc")
                          //  ->take(200)
                            ->get();            
            //echo "Empezamos captura de datos"."\n";
            $tmp = [];
            $id_old = $results[0]->id;
    ...
    private function getDatosCompetencias($result_id, $competencias)
    {
        ...
        foreach($competencias as $dimension => $competencia)
        {
            foreach($competencia as $name_competencia => $slides)
            {
                ...
                foreach($slides as $slide_id)
                {
                   // echo "consulta: ".$dimension." ".$name_competencia." ".$slide_id."\n";
                    if($name_competencia != "Visión Estratégica")
                    $r = Answer::where("result_id", $result_id)
                        ...
                        $r2 = Answer::where("result_id", $result_id)
                            ->where("question_dimension", $dimension)
                            ->where("duplicated",0)
                            ->where("question_competencia", $name_competencia)
                            ->where("slide_id", $slide_id)
