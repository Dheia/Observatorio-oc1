...
class Equipo extends Model
{
     ...
     */
    public $table = 'tresfera_skillyouup_equipo';

    public $jsonable = ['tipo','players','params','stats'];

    public $hasOne = [
      'result' => [
        'Tresfera\Skillyouup\Models\Result',
        'conditions' => 'is_autoevaluacion = 1'
      ],
      'rapport' => [
        'Tresfera\Skillyouup\Models\Rapport',
      ]
    ];
    ...
    public $belongsToMany = [
      'evaluadores' => [
        'Backend\Models\User',
        'table' => 'tresfera_skillyouup_evaluacion_evaluador',
        'key' => 'eval',
        'otherKey' => 'evaluador'
      ]
    ];
    public $belongsTo = [
      'proyecto' => [
        'Tresfera\Skillyouup\Models\Proyecto'
      ]
    ];
    ...
    public function isCompletedPlayer($email) {
      $a = \Tresfera\Skillyouup\Models\Result::where("evaluacion_id",$this->id)->where("is_evaluacion", 1)->where("email", $email)->count();
      if($a > 0) return true;
      return false;
    }

    public function getDatePlayer() {
      $a = \Tresfera\Skillyouup\Models\Result::where("evaluacion_id",$this->id)->where("is_autoevaluacion", 1)->first();
    ...
    public function beforeCreate() {
      ...
      else
      {
        ...
        {
          \Flash::success(e(trans('tresfera.skillyouup::lang.evaluadores.save_ok')));
        }
        else
        {
          \Flash::error( e(trans('tresfera.skillyouup::lang.evaluadores.save_ko')). json_encode($validacion["emails_error"]));
        }
        return true;
      }
    ...
    public function afterSave() {
      ...
      if(isset($evaluacion->id)) {
        ...
        if( count($eval_tmp["pendientes"])==0)
        {
            $this->estado = 2;
            $this->save();
            \Queue::push('\Tresfera\Skillyouup\Classes\Jobs\GenerateRapport',$this->id,"rapports");
        }
        else
        {
            $this->estado = 1;
    ...
    public function afterCreate() {
     ...
     /* $user = User::where("login",$username)->first();
      ...
      if($this->lang == "en")
        $theme = 'skillyouup.require.aprovacion_en';
    ...
    public function beforeUpdate() {
      ...
      //if($this->estado==1) {
        ...
        if(is_array($this->players)) {
          foreach($this->players as $contacto) {
            if(isset($stats[$contacto['email']])) continue;
            $stats[$contacto['email']] = [
                                                  'name' => $contacto['name'],
                                                  'email'=> $contacto['email'],
                                                  'url' => "",
    ...
    public function getUrl() {
      ...
      if( $this->lang == 'en') {
        $url_base = "/quiz/skillyouup-en";
      } else {
        $url_base = "/quiz/skillyouup";
      }
      return ($url_base."?skillyouup=1&ev=".$this->id);
    ...
    public function listPlayersAnswereds($player) {
      $resultBase = Result::select(\DB::raw("value, count(value) as num"))
                            ...
                            ->where("tresfera_skillyouup_answers.duplicated",0)
                            ->where("email",$player)
