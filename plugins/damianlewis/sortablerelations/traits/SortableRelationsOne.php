...
trait SortableRelations
{
                ...
                if ($relationName == $sortableRelationName) {
                    foreach ($pivotIdsAttributes as $id => $attributes) {
                        ...
                        $relation = $model->{$relationName}()
                            ->where('tresfera_flex360_slides.id', $id)
                            ->first();
                        ...
                        if ($relation->relation_sort_order > $target->relation_sort_order) {
                            $model->shiftRelations($relationName, $target, false, false, $relation->relation_sort_order);
                        } else {
                            $model->shiftRelations($relationName, $target, true, true, $relation->relation_sort_order);
                        }
    ...
    protected function shiftRelations(
        ...
        if (!is_null($untilSortOrder) && $relation->relation_sort_order == $untilSortOrder) {
            return;
        }
        ...
        $isLeftShift ? $relation->relation_sort_order -= 1 : $relation->relation_sort_order += 1;
        $relation->save();
    }
    ...
    protected function getAdjacentRelation($relationName, $relation, $isPrevious = false)
    {
        ...
        return $this->{$relationName}()
            ->where('relation_sort_order', $relation->relation_sort_order + (1 * ($isPrevious ? -1 : 1)))
            ->first();
    }
